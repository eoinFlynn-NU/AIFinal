...
class Agent:
  ...
  def save_model(self, save_file):
    torch.save(self.target_model.state_dict(), save_file)
  
  def load_model(self, load_file):
    self.model.load_state_dict(torch.load(load_file))
    self.model.eval()

def train_space_invaders_agent(num_episodes, save_file):
...
def evaluate_space_invaders_agent(num_episodes, weight_file):
    model = SpaceInvadersNet(1, actions)
...
# weight_file = './trained_weights/space_invaders_10T.pth' # path to weight file for evaluation 
# evaluate_space_invaders_agent(50, weight_file)